...
/**
 * Convert bytes into MB and round it to two decimals
 *
 * @param bytes {Number}
 *
 * @returns {Number}
...
/**
 * Collect memory stats and convert them into MB to easy readability
 * In order to better understand these numbers the following link 
 * can be used as a reference: https://stackoverflow.com/questions/12023359/what-do-the-return-values-of-node-js-process-memoryusage-stand-for
 *
 * @returns {Object}
 */
...
var Client = function (config, logger) {
    ...
    if (this.systemStats) {
        blocked(function (ms) {
            var memory = collectMemoryStats();
            var timestamp = Date.now();
            var tags = { unit: 'mb' };

            self.timer('event_loop', ms);
            self.put('memory_rss', memory.rss, timestamp, tags);
            self.put('memory_heap_total', memory.heapTotal, timestamp, tags);
            self.put('memory_heap_used', memory.heapUsed, timestamp,  tags );
            self.put('memory_external', memory.external, timestamp, tags);
        });
    }
